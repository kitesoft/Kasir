...
uses
  ...
  StdCtrls, sLabel, Mask, sMaskEdit, sCustomComboEdit, sCurrEdit, sCurrencyEdit,
  math, mySQLDbTables, sButton, sGroupBox, sEdit, inifiles;
...
begin
  ...
  try
    fungsi.SQLExec(dm.Q_exe, 'call sp_setor_kasir("' + f_transaksi.sb.Panels[1].Text
      + '","' + f_transaksi.Sb.Panels[2].Text + '","' + F_transaksi.Sb.Panels[4].Text
      + '")', False);
    dm.My_Conn.Commit;
  except
    on e: exception do
  ...
  fungsi.SQLExec(dm.Q_temp, 'select * from tb_login_kasir where kd_perusahaan="'
    + f_transaksi.sb.Panels[1].Text + '" and user="' + F_transaksi.Sb.Panels[2].Text
    + '"  and kd_jaga = "' + F_transaksi.Sb.Panels[4].Text +
...
procedure TF_Jual_Kasir.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if key = vk_escape then
    if gb_tutup.Visible = false then
      close;
end;
