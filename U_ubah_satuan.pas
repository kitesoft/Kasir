...
begin
  fungsi.SQLExec(dm.Q_show,
    'SELECT tb_barang.n_barang,tb_barang.kd_sat1,tb_barang.kd_sat2,tb_barang.kd_sat3, ' +
  ...
  fungsi.SQLExec(dm.Q_Temp, 'select * from tb_satuan where kd_satuan="' + sat3 +
    '"', true);
  ...
  fungsi.SQLExec(dm.Q_Temp, 'select * from tb_satuan where kd_satuan="' + sat2 +
    '"', true);
  ...
  fungsi.SQLExec(dm.Q_Temp, 'select * from tb_satuan where kd_satuan="' + sat1 +
    '"', true);
  TableView.DataController.SetValue(2, 0, dm.Q_show.fieldbyname('barcode1').AsString);
  TableView.DataController.SetValue(2, 1, dm.Q_show.fieldbyname('Qty1').AsInteger
    * dm.Q_show.fieldbyname('Qty2').AsInteger);
...
procedure TF_Ubah_Satuan.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
var
  Qty_real: Integer;
begin
  Qty_real := F_Transaksi._Get(0, 3, 1) * TableView.DataController.GetValue(TableView.DataController.FocusedRecordIndex,
    1);

  F_Transaksi._set(0, 4, 1, TableView.DataController.GetValue(TableView.DataController.FocusedRecordIndex,
    2));
  F_Transaksi._set(0, 5, 1, TableView.DataController.GetValue(TableView.DataController.FocusedRecordIndex,
    3));

  F_Transaksi._set(0, 7, 1, (F_Transaksi._get(0, 7, 1) / F_Transaksi._get(0, 18,
    1)) * Qty_real); //diskon Rp
  F_Transaksi._set(0, 6, 1, (F_Transaksi._get(0, 7, 1) * 100) / F_Transaksi._get
    (0, 5, 1)); //diskon pr
  F_Transaksi._set(0, 8, 1, F_Transaksi._Get(0, 5, 1) - F_Transaksi._get(0, 7, 1));
    //netto
  F_Transaksi._set(0, 9, 1, F_Transaksi._Get(0, 8, 1) * F_Transaksi._Get(0, 3, 1));
    //netto
  F_Transaksi._set(0, 14, 1, F_Transaksi._Get(0, 13, 1) * Qty_real); //total harga pokok
  F_Transaksi._set(0, 15, 1, F_Transaksi._Get(0, 9, 1) - F_Transaksi._Get(0, 14,
    1)); //total laba
  F_Transaksi._set(0, 16, 1, TableView.DataController.GetValue(TableView.DataController.FocusedRecordIndex,
    4));
  F_Transaksi._set(0, 18, 1, Qty_real);

  F_Transaksi._set(0, 19, 1, F_Transaksi._Get(0, 19, 1) + 'ubah satuan, Qty = '
    + inttostr(Qty_real) + ', '); //keterangan ubah manual
